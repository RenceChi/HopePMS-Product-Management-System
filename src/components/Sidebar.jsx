...
// 1. The Raw Data (Defined outside so it doesn't re-create on every render)
const navGroups = [
  {
    section: 'Main',
    items: [
  ...
  },
  {
    ...
    items: [
      ...
      // 🔒 Mark as Deleted Items
      { label: 'Deleted Items', to: '/deleted-items', isDeletedItems: true,
  ...
  {
    section: 'Admin',
    isAdminSection: true, // 🔒 Mark for gating
    items: [
...
function SidebarContent({ onClose, isMobile }) {
  const { canAccessAdmin, canViewDeleted } = useRights();

  // 🔒 This MUST be inside SidebarContent so it can use the variables above
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden">
              ...
              {group.items.map(item => (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    ...
                    ${isActive ? 'font-semibold shadow-[0_2px_10px_rgba(49,81,30,0.18)]' : 'font-medium hover:bg-[rgba(133,159,61,0.09)]'}`
                  }
                  style={({ isActive }) => ({
      ...
      <div className="px-4 py-3 flex items-center gap-2" style={{ borderTop: '1px solid rgba(133,159,61,0.1)' }}>
        <div className="w-1.25 h-1.25 rounded-full" style={{ background: '#859F3D' }} />
