...
export default function Sidebar({ open, isMobile, onClose, overrideUserType }) {
  if (isMobile) {
    return (
      <>
        {open && (
        ...
        >
          <SidebarContent onClose={onClose} isMobile overrideUserType={overrideUserType} />
        </div>
      </>
    );
  }
  ...
  return (
    ...
    >
      <SidebarContent overrideUserType={overrideUserType} />
    </div>
  );
}

function SidebarContent({ onClose, isMobile, overrideUserType }) {
  const { currentUser } = useAuth();
  const userType = overrideUserType ?? currentUser?.user_type ?? 'USER';
